...
private:
    ...
    inline T update_block(int block)
    {
        ...
        for (int i = block*BlockSize; i <= (block+1)*BlockSize - VectorSize; i+=VectorSize)
        {
            ...
            vxi = add(vxi, mul(vtimeStepSize, condset(div(fxi, massi), vzero, cond_nz_massi)));
            vyi = add(vyi, mul(vtimeStepSize, condset(div(fyi, massi), vzero, cond_nz_massi)));
            vzi = add(vzi, mul(vtimeStepSize, condset(div(fzi, massi), vzero, cond_nz_massi)));
            store(vx+i, vxi);
            store(vy+i, vyi);
            store(vz+i, vzi);

            V vV2 = add(mul(vxi, vxi), add(mul(vyi, vyi), mul(vzi, vzi)));
            vV2 = condset(vV2, vzero, cond_nonzero(massi));
            vmaxV2 = vmax(vmaxV2, vV2);
        }
        return hmax(vmaxV2);
    }
...
public:
    ...
    inline void updateBody() {
        ...
        T minCollisionCondition  = std::numeric_limits<T>::max();


        clear_forces();

        T* round_listminDx = new T[NumberOfBlocks];
        T* round_listminCollisionCondition = new T[NumberOfBlocks];

        // Round-robin tournament (https://en.wikipedia.org/wiki/Round-robin_tournament)
        for (int i = 0; i < NumberOfBlocks-1; i++)
        {
            // Given i, for each j we have dosjoint pair of blocks a(j), b(j), and we can work on them in parallel
            #pragma omp parallel for
            for (int j = 0; j < NumberOfBlocks/2; j++)
            {
                V my_vminDistance = vnumericLimitMax;
                V my_vminCollisionCondition = vnumericLimitMax;
                int a = (j == 0) ? 0 : ((i+j)%(NumberOfBlocks-1))+1;
                int b = ((i+NumberOfBlocks-j-1)%(NumberOfBlocks-1))+1;
                process_distinct_blocks(a, b, my_vminDistance, my_vminCollisionCondition);
                round_listminDx[j] = hmin(my_vminDistance);
                round_listminCollisionCondition[j] = hmin(my_vminCollisionCondition);
            }

            T round_minDx = *(std::min_element(round_listminDx, round_listminDx + NumberOfBlocks/2));
            T round_minCollisionCondition = *(std::min_element(round_listminCollisionCondition, round_listminCollisionCondition + NumberOfBlocks/2));

            minDx = std::min(minDx, round_minDx);
            minCollisionCondition = std::min(minCollisionCondition, round_minCollisionCondition);
        ...
        for (int i = 0; i < NumberOfBlocks; i++)
        {
            V my_vminDistance = vnumericLimitMax;
            V my_vminCollisionCondition = vnumericLimitMax;
            process_single_block(i, my_vminDistance, my_vminCollisionCondition);
            round_listminDx[i] = hmin(my_vminDistance);
            round_listminCollisionCondition[i] = hmin(my_vminCollisionCondition);
        }
        T round_minDx = *(std::min_element(round_listminDx, round_listminDx + NumberOfBlocks));
        T round_minCollisionCondition = *(std::min_element(round_listminCollisionCondition, round_listminCollisionCondition + NumberOfBlocks));
        ...
        // not much benefit in running in parallel
        T maxV2 = 0;
        for (int i = 0; i < NumberOfBlocks; i++)
        {
        ...
        t += timeStepSize;
        delete [] round_listminDx;
        delete [] round_listminCollisionCondition;
    }
