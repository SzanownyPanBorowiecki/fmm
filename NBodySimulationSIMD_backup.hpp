...
//            printvec(distance);
    ...
    inline void process_distinct_blocks(int blocki, int blockj, V &minDistance, V &minCollisionCondition) 
    {
        ...
        for (int i = blocki*BlockSize; i <= std::min((blocki+1)*BlockSize, ArraySize) - VectorSize; i+=VectorSize)
        {
            ...
            for (int j = blockj*BlockSize; j <= std::min((blockj+1)*BlockSize, ArraySize) - VectorSize; j+=VectorSize)
            {
                ...
                process_distinct_vectors(xxi, xyi, xzi, massi, xxj, xyj, xzj, massj, fxi, fyi, fzi, fxj, fyj, fzj, singlevector_minDistance, singlevector_minCollisionCondition);
    ...
    inline void process_single_block(int blocki, V &minDistance, V &minCollisionCondition)
    {
        ...
        for (int i = blocki*BlockSize; i <= std::min((blocki+1)*BlockSize, ArraySize) - VectorSize; i+=VectorSize)
        {
            ...
            V singlevector_minDistance, singlevector_minCollisionCondition;
            process_single_vector(xxi, xyi, xzi, massi, fxi, fyi, fzi, singlevector_minDistance, singlevector_minCollisionCondition);
            m_minDistance = vmin(m_minDistance, singlevector_minDistance);
            m_minCollisionCondition = vmin(m_minDistance, singlevector_minCollisionCondition);
    ...
    inline void updateBody() {
        ...
        for (int i = 0; i < NumberOfBlocks-1; i++)
        {
            for (int j = 0; j < NumberOfBlocks/2; j++)
            {
                ...
                V vBlockMinDistance;
                V vBlockMinCollisionCondition;
                process_distinct_blocks(a, b, vBlockMinDistance, vBlockMinCollisionCondition);
                vMinDistance = vmin(vMinDistance, vBlockMinDistance);
                vMinCollisionCondition = vmin(vMinCollisionCondition, vBlockMinCollisionCondition);
            }
        }
        ...
        for (int i = 0; i < NumberOfBlocks; i++)
        {
            V vBlockMinDistance;
            V vBlockMinCollisionCondition;    
            process_single_block(i, vBlockMinDistance, vBlockMinCollisionCondition);
            vMinDistance = vmin(vMinDistance, vBlockMinDistance);
            vMinCollisionCondition = vmin(vMinCollisionCondition, vBlockMinCollisionCondition);
        }
        ...
        //std::cout << "minColCond=" << minCollisionCondition << std::endl;
